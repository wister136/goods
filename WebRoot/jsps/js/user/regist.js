$(function() {
	/*
	 * 1.得到所有的错误信息，循环遍历之，调用一个方法来确定是否显示错误信息 文件加载完毕才运行这个
	 */
	...
	$(".inputClass").focus(function() {
		var labelId = $(this).attr("id") + "Error";// 通过输入框找到队形的label的id
...
function validateLoginname() {
	...
	$.ajax({
		...
		type : "POST",
		dataType : "json",
		async : false,// 是否异步请求，如果异步，那么不等服务器返回，我们这个函数就想下运行了
		cache : false,
...
function validateEmail() {
	...
	$.ajax({
		url : "/goods/UserServlet",// 要请求的servlet
		data : {
			method : "ajaxValidateEmail",email : value
		},// 给服务器的参数
		type : "POST",
		dataType : "json",
		async : false,// 是否异步请求，如果异步，那么不等服务器返回，我们这个函数就想下运行了
...
function validateVerifyCode() {
	...
	$.ajax({
		url : "/goods/UserServlet",// 要请求的servlet
		data : {
			method : "ajaxValidateVerifyCode",verifyCode : value
		},// 给服务器的参数
		type : "POST",
		dataType : "json",
		async : false,// 是否异步请求，如果异步，那么不等服务器返回，我们这个函数就想下运行了
...
function _hyz() {
	/*
	 * 1.获取<img>元素 2.重新设置他的src 3.使用毫秒来添加参数
	 */
