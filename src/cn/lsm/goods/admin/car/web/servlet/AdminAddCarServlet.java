...
public class AdminAddCarServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp)
		 ...
		 */
		// 创建fileItem对象的工厂，在这个工厂中可以配置内存缓冲区的大小和存放临时文件的目录
		...
		sfu.setFileSizeMax(8000 * 1024);//设置单个上传的文件上限为80KB
		/*
		...
		try {
			...
			error("上传的文件超出了80KB", req, resp);
			return;
		}
		
		/*
		 * 4. 把List<FileItem>封装到Car对象中
		...
		// 获取文件名
		 fileItem = fileItemList.get(2);//获取大图
		 ...
		 * 1. 获取真实路径
		 */
//		 savepath=new File(getServletContext().getRealPath("upload"));
//		System.out.println("uploading-----"+savepath);
//		if(!savepath.exists()){
//			savepath.mkdir();
//		}
...
//		}
		...
		// 调用service完成保存
		car.setBid(CommonUtils.uuid());
		CarService bookService = new CarService();
		bookService.add(car);
	...
	public void error(String msg, HttpServletRequest req,
		...
		req.getRequestDispatcher("/adminjsps/admin/book/add.jsp").forward(req,
				resp);
	}
